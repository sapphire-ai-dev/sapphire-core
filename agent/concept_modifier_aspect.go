...
func (a *Agent) newAspectModifier(t *aspectModifierType, target concept,
	params map[string]any, source int) *aspectModifier {
	result := &aspectModifier{
		params: params,
	}

	a.newAbstractModifier(result, t, target, &result.abstractModifier, source)
...
func (t *aspectModifierType) instantiate(target concept, source int, args ...any) modifier {
	...
	return t.agent.newAspectModifier(t, target, params, source).memorize().(*aspectModifier)
}
...
func (a *Agent) newAspectModifierType(aspect *aspectNode) *aspectModifierType {
	result := &aspectModifierType{
		aspect: aspect,
	}

	a.newAbstractModifierType(result, &result.abstractModifierType)
