...
func TestWordLangPartFit(t *testing.T) {
	...
	class, word := toReflect[*simpleObject](), "it"
	soWlp := agent.language.newLangNode(class).newLangForm().newWordLangPart(class, word)
	soSrc, soDst := agent.newSimpleObject(1), agent.newSimpleObject(2)
...
func TestConceptLangPartFit(t *testing.T) {
	...
	soSbj := agent.newSimpleObject(456)
...
func TestRecursiveLangPartInstantiate(t *testing.T) {
	...
	redType := agent.newAspectModifierType(agent.aspect.find(world.InfoLabelObservable, "color", word))
	redInst := agent.newAspectModifier(redType, tc, map[string]any{}, conceptSourceObservation)
