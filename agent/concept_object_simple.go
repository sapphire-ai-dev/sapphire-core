...
func (a *Agent) newSimpleObject(worldId int) *simpleObject {
...
func (a *Agent) newSimpleObjectType(source int, modifTypes map[int]modifierType) *simpleObjectType {
	result := &simpleObjectType{}
	a.newAbstractObjectType(result, source, &result.abstractObjectType)
...
func (a *Agent) interpretSimpleObjectType(_ map[int]concept, _ ...any) concept {
	result := &simpleObjectType{}
	a.newAbstractObjectType(result, conceptSourceLanguage, &result.abstractObjectType)
	result.unique = true
	return result.memorize().(*simpleObjectType)
}
