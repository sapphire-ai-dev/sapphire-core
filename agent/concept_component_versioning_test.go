...
func TestVersioningOverlapSingleSide(t *testing.T) {
	...
	amt1.instantiate(so, conceptSourceObservation, map[int]any{partIdConceptTime: tsos[1][4]})
	...
	amt2.instantiate(so, conceptSourceObservation, map[int]any{partIdConceptTime: tsos[0][2]})
	am1 = assertAspectModifierSeen(t, so.modifiers(nil), amt1, so)
	...
	amt2.instantiate(so, conceptSourceObservation, map[int]any{partIdConceptTime: tsos[3][5]})
	am1 = assertAspectModifierSeen(t, so.modifiers(nil), amt1, so)
	assert.Equal(t, am1.time().start(), tpos[2])
...
func TestVersioningOverlapSingleSideOriginalNils(t *testing.T) {
	...
	amt1.instantiate(so1, conceptSourceObservation, map[int]any{
		partIdConceptTime: agent.newTimeSegmentObject(nil, tpos[3], nil),
	})
	am1 := assertAspectModifierSeen(t, so1.modifiers(nil), amt1, so1)
	assert.Nil(t, am1.time().start())
	assert.Equal(t, am1.time().end(), tpos[3])

	amt2.instantiate(so1, conceptSourceObservation, map[int]any{partIdConceptTime: tsos[0][4]})
	am1 = assertAspectModifierSeen(t, so1.modifiers(nil), amt1, so1)
	assert.Nil(t, am1.time().start())
	...
	amt2.instantiate(so2, conceptSourceObservation, map[int]any{partIdConceptTime: tsos[0][4]})
	am3 = assertAspectModifierSeen(t, so2.modifiers(nil), amt3, so2)
