...
func (a *Agent) newActionStateChange(t *actionStateChangeType, target performableAction) *actionStateChange {
	if target.state() != actionStateDone {
		return nil
	}

	result := &actionStateChange{}
	a.newAbstractChange(result, t, nil, &result.abstractChange)
...
func (a *Agent) newActionStateChangeType(target performableActionType) *actionStateChangeType {
	result := &actionStateChangeType{}
	a.newAbstractChangeType(result, &result.abstractChangeType)
	result.target = target.createReference(result, true)
