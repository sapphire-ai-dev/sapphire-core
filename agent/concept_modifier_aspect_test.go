...
func TestAspectModifierGroup(t *testing.T) {
	...
	am1 := amt1.instantiate(so, conceptSourceGeneralization, map[int]any{partIdConceptTime: tpos[0]})
	am2 := amt2.instantiate(so, conceptSourceGeneralization, map[int]any{partIdConceptTime: tpos[1]})
...
func TestAspectModifierVersionInterrupt(t *testing.T) {
	...
	amt1.instantiate(so, conceptSourceGeneralization, map[int]any{partIdConceptTime: tsos[1][6]})
	assert.Len(t, so.modifiers(nil), 1)
	amt2.instantiate(so, conceptSourceGeneralization, map[int]any{partIdConceptTime: tsos[3][4]})
