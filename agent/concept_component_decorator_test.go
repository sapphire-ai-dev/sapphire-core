...
func TestDecoratorGetRelations(t *testing.T) {
	...
	am1 := amt1.instantiate(so, conceptSourceObservation, map[int]any{conceptArgTime: tso12})
	am2 := amt2.instantiate(so, conceptSourceObservation, map[int]any{conceptArgTime: tso23})
	assert.NotEqual(t, am1, am2)
	assert.Len(t, so.modifiers(nil), 2)
	assert.Empty(t, so.modifiers(map[int]any{conceptArgTime: tpo0}))
	assert.Len(t, so.modifiers(map[int]any{conceptArgTime: tpo1}), 1)
	assert.Len(t, so.modifiers(map[int]any{conceptArgTime: tpo2}), 2)
	assert.Len(t, so.modifiers(map[int]any{conceptArgTime: tpo3}), 1)
	assert.Empty(t, so.modifiers(map[int]any{conceptArgTime: tpo4}))
	assert.Len(t, so.modifiers(map[int]any{conceptArgTime: tso01}), 1)
	assert.Len(t, so.modifiers(map[int]any{conceptArgTime: tso02}), 2)
	assert.Len(t, so.modifiers(map[int]any{conceptArgTime: tso04}), 2)
	assert.Len(t, so.modifiers(map[int]any{conceptArgTime: tso13}), 2)
	assert.Len(t, so.modifiers(map[int]any{conceptArgTime: tso24}), 2)
	assert.Len(t, so.modifiers(map[int]any{conceptArgTime: tso34}), 1)
}
